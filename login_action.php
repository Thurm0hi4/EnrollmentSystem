...
if($values = oci_fetch_array ($cursor)){
  ...
  $sql = "insert into myclientsession " .
    "(sessionid, clientid, sessiondate, admin) " .
    "values ('$sessionid', '$clientid', sysdate, '$admin')";
  ...
  if ($result == false){
    ...
    if ($admin == 1){
      // insert OK - we have created a new session open admin page
      header("Location:adminwelcome.php?sessionid=$sessionid");
    }
    else { //insert OK - open student page
      header("Location:stuwelcome.php?sessionid=$sessionid");
    }
  }
}
else {
